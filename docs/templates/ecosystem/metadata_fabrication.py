...
REPOS = {
    ...
    'forj': {
        ...
        'upstream_deps': [
            'pravara-mes (WIRED — POST /v1/orders dispatch on PAID orders + status-back webhook)',
            'janua (creator + shopper auth; also mints the yantra4d render token)',
            'yantra4d + fashion-cabinet (parametric commons materialized into the catalog)',
            'dhanam (payments — real-money checkout returns 501 until the Connect surface ships)',
            'on-chain L2 (NFT minting)',
            'digifab-quoting / Cotiza (PLANNED, not wired — no quoting call exists; forj stores an STL per product per its contract)',
        ],
        'downstream_consumers': [
            'pravara-mes (fab jobs from PAID orders)',
            'phynd-crm (order federation into customer portal)',
        ],
        'key_env': [
            'DATABASE_URL — Postgres',
            'JANUA_JWKS_URL / JANUA_CLIENT_ID / JANUA_CLIENT_SECRET — auth',
            'PRAVARA_API_URL / PRAVARA_API_TOKEN — fab dispatch (unset token ⇒ visible skip)',
            'PRAVARA_WEBHOOK_SECRET — status-back HMAC (fail-closed; unset ⇒ 401)',
            'YANTRA4D_API / FC_API — commons catalog sources',
            'FORJ_YANTRA4D_CLIENT_ID / FORJ_YANTRA4D_CLIENT_SECRET — render token (scope yantra4d:render)',
            'ASSET_PUBLIC_BASE — origin serving GET /api/assets/<key>',
