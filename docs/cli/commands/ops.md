---
title: ops
description: Audited MADFAM operator workflows for Kubernetes, Argo, storage, policy, and runners
---

# `enclii ops`

`enclii ops` is the contract-first replacement layer for routine direct
`kubectl`, Argo, Longhorn, ExternalSecrets, Kyverno, and ARC operations.
...
diagnosis/logs, PVC/PV inventory, Longhorn CRDs, ExternalSecrets, Vault pod
readiness, Kyverno reports/exceptions, and ARC runner-scale-set inventory. If an
...
- `apps sync|rollback`, `pods restart`, `storage repair-plan`, `secrets refresh`,
