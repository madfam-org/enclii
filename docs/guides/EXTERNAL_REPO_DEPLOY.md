...
| Active deployments stuck "Deploying" forever | CI commit SHA (A) differs from ArgoCD sync SHA (B) due to digest-commit creating a new commit | Fixed: callback falls back to time-window lookup (30 min) when SHA match fails. Active Deployments card (always visible) filters out deploying records older than 30 min and shows green idle state when empty |
