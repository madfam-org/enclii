...
func newOpsAppsCommand(cfg *config.Config) *cobra.Command {
	...
	cmd.AddCommand(newOpsActionCommand(cfg, "apps", "sync-sweep", "Sync all drifted Argo apps except known exclusions (GA O-8)"))
...
func newOpsStorageCommand(cfg *config.Config) *cobra.Command {
	cmd := &cobra.Command{Use: "storage", Short: "PVC, PV, Longhorn, and object-storage workflows"}
	...
	cmd.AddCommand(newOpsReadCommand(cfg, "storage", "r2-audit", "Audit R2 credentials: incomplete, shared, or mismatched buckets"))
