...
| react-sdk pre-existing test failures | Known debt | 63 tests across 3 suites in `@janua/react-sdk` (`__tests__/SignIn`, `__tests__/SignUp`, `__tests__/UserProfile`) — `@janua/ui` moduleNameMapper mocks render empty divs so tests can't find form elements, and `useJanua` mock returns incomplete context. 9/12 suites pass (60/123 tests). Barrel export test (47 tests) + hook tests + component stubs all pass. `\|\| true` removed in Session 95 |
