#!/usr/bin/env bash
# Restore crm.phynd.app through Enclii-managed DNS paths.
#
# Dry-run by default. Use --apply only after the apex authority and required
# provider credentials are present through approved secret management. Secret
# values are never printed by this script.

set -euo pipefail

TARGET="${TARGET:-crm.phynd.app}"
DOMAIN="${DOMAIN:-phynd.app}"
RECORD_TYPE="${RECORD_TYPE:-CNAME}"
RECORD_CONTENT="${RECORD_CONTENT:-c9fac286-497b-4aac-9288-f784a1ea561c.cfargotunnel.com}"
REASON="${REASON:-restore PhyndCRM app host through Enclii}"
...
usage() {
  cat <<'EOF'
Usage: scripts/remediate-phynd-app-host.sh [--apply|--dry-run] [--skip-vault-repair]

Environment:
  TARGET          Host to restore. Default: crm.phynd.app
  DOMAIN          Apex domain. Default: phynd.app
...
The script exits before DNS mutations if RDAP cannot verify the apex domain.
EOF
}
...
assert_domain_ready() {
  ...
  if [[ "$registered_rc" -ne 0 ]]; then
    ...
    die "${DOMAIN} is not verified through RDAP; confirm registrar ownership and delegation before DNS apply"
  fi
