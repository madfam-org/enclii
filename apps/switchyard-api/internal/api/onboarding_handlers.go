...
func (h *Handler) OnboardRepo(c *gin.Context) {
	...
	// Step 5: Auto-commit ArgoCD Application YAML to Enclii repo
	...
	if h.config.GitHubToken != "" && h.config.EncliiRepoOwner != "" && h.config.EncliiRepoName != "" {
		argocdFilePath := fmt.Sprintf("infra/argocd/apps/%s.yaml", appName)
		commitMsg := fmt.Sprintf("feat(argocd): auto-onboard %s\n\nGenerated by POST /v1/admin/onboard for %s", appName, req.RepoFullName)
		...
		if argocdCommitErr != nil {
			h.logger.Warn(ctx, "Failed to auto-commit ArgoCD app (non-fatal, YAML returned in response)",
				logging.String("app", appName),
				logging.Error("error", argocdCommitErr))
		} else {
			h.logger.Info(ctx, "Auto-committed ArgoCD app to Enclii repo",
				logging.String("app", appName),
	...
	nextSteps := []string{}
	if argocdCommitSHA != "" {
		nextSteps = append(nextSteps, fmt.Sprintf("1. ArgoCD app auto-committed (%s)", argocdCommitSHA[:8]))
	} else {
		nextSteps = append(nextSteps, fmt.Sprintf("1. Commit ArgoCD app YAML to infra/argocd/apps/%s.yaml", appName))
	}
