...
func (h *Handler) OnboardRepo(c *gin.Context) {
	...
	// Step: Generate and commit NetworkPolicies (non-critical)
	if encliiConfig != nil && encliiConfig.Spec.Network != nil && h.config.GitHubToken != "" {
		...
		if npGenErr != nil {
			...
			npFilePath := fmt.Sprintf("infra/k8s/policies/%s-network-policies.yaml", req.ProjectName)
			npMsg := fmt.Sprintf("feat(netpolicy): auto-generate NetworkPolicies for %s\n\nGenerated by POST /v1/admin/onboard from enclii.yaml network section.", req.ProjectName)
			_, npErr := createOrUpdateGitHubFile(ctx, h.config.GitHubToken, h.config.EncliiRepoOwner, h.config.EncliiRepoName, npFilePath, npYAML, npMsg, "main")
			h.recordStep(ctx, &steps, "network_policies", false, npErr)
		}
	}
