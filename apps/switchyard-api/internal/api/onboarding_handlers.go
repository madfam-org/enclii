...
func (h *Handler) OnboardRepo(c *gin.Context) {
	...
	if len(statusEntries) > 0 {
		configSnapshot["status_entries"] = statusEntries
	}
...
// registerStatusEntries projects status page entries from enclii.yaml into the
// onboarding snapshot. It intentionally does not edit Enclii-owned ConfigMaps;
// status config is projected later from onboarding DB state.
func (h *Handler) registerStatusEntries(ctx context.Context, projectName string, config *manifest.EncliiYAML) ([]statusServiceEntry, error) {
	...
	manifestEntries := status.Entries
	if len(manifestEntries) == 0 && len(config.Spec.Domains) > 0 {
		for _, d := range config.Spec.Domains {
			manifestEntries = append(manifestEntries, manifest.EncliiYAMLStatusEntry{
				Name:  d.Name,
				URL:   "https://" + d.Name,
				Group: projectName,
			})
	...
	entries := make([]statusServiceEntry, 0, len(manifestEntries))
	...
	if h.logger != nil {
		h.logger.Info(ctx, "Recorded status page entries in onboarding snapshot",
