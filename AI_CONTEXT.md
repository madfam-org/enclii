...
- **Where to add the annotation**:
  - Plain manifest: under `spec.template.metadata.annotations`
  - Kustomize: as a JSON-Patch using the JSON-Pointer escape `~1` for `/`: `path: /spec/template/metadata/annotations/reloader.stakater.com~1match`
- **Tier 2 expansion** (one PR per ecosystem repo) is the right model — do NOT add ecosystem deployment annotations in the enclii repo
- **Runbook**: `docs/runbooks/CONFIG_RELOAD_RUNBOOK.md` (test procedure, troubleshooting, rollback)
...
- **Control-plane node** - Control plane + primary workload node (dedicated bare-metal) — provisioned 2026-04-08
- **Worker node** - Worker + Longhorn 2nd replica target (dedicated bare-metal)
- **Builder node** - CI builds only (cloud compute instance, taint: builder=true:NoSchedule)
...
> **Note:** 3-node cluster since Apr 2026. Control-plane migrated onto dedicated bare-metal on 2026-04-08. The control-plane node runs k3s server + production workloads; the Longhorn replica worker handles spillover workloads and serves as Longhorn 2nd replica target. Builder node runs only ARC GitHub Actions runners. Longhorn CSI operates in 2-replica mode for storage redundancy across dedicated nodes.
...
| Purpose | Location |
|---------|----------|
| Middleware (server-side auth) | `apps/admin-console/middleware.ts` |
| Auth helpers (pure functions) | `apps/admin-console/lib/auth-helpers.ts` |
| Auth Context (client-side) | `apps/admin-console/contexts/AuthContext.tsx` |
| K8s Deployment | `apps/admin-console/k8s/deployment.yaml` |
| Dockerfile | `apps/admin-console/Dockerfile` |
| Analytics | `apps/admin-console/lib/analytics/posthog.ts`, `apps/admin-console/components/PostHogProvider.tsx` |
| Unit tests | `apps/admin-console/__tests__/` (10 files, 123 tests: auth, API, Cloudflare, components, analytics) |
...
- ✅ `selva-nexus-api` → agents-api.madfam.io (agent orchestration API)
- ✅ `selva-office-ui` → agents.madfam.io (agent management console)
- ✅ `selva-admin` → agents-admin.madfam.io (admin console)
- ✅ `selva-colyseus` → agents-ws.madfam.io (real-time collaboration)
- ✅ `selva-gateway` → (background worker, no public endpoint)
- ✅ `selva-workers` → (background worker, no public endpoint)
